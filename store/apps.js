...
export const actions = {
  async fetch({ commit }) {
    const response = await this.$axios.get(`/api/apps`);
  ...
  async fetchPublishers({ commit }) {
    const response = await this.$axios.get(`/api/app-publishers`);
    const content = response.data;
    commit("setPublishers", content);
  },
  async fetchCategories({ commit }) {
    const response = await this.$axios.get(`/api/app-categories`);
    const content = response.data;
