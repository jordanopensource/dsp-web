      ...
      <article v-if="app" class="container py-12">
        <div class="content">
          <div class="flex flex-col sm:flex-row flex-nowrap items-start mb-12">
            ...
            <div class="mt-4 sm:mt-0 sm:ltr:ml-8 sm:rtl:mr-8">
              ...
              <NuxtLink v-if="app.app_publisher" :to="publisherLink">
                <h3 class="my-2">
                  {{ app.app_publisher["title_" + $i18n.locale] }}
                </h3>
              </NuxtLink>
              <div class="mt-4 sm:mt-8 flex flex-col md:flex-row">
                <span class="ltr:mr-4 rtl:ml-4">
                  <template v-if="app.open_source">
                    <i
                      class="ri-open-source-fill text-2xl align-middle text-josa-teal"
                    ></i
                    ><span class="text-sm mx-1 align-middle">{{
                      $t("openSource")
                    }}</span>
                  </template>
                  <template v-else>
                    <i
                      class="ri-open-source-fill text-2xl align-middle display-faded"
                    ></i
                    ><span class="text-sm mx-1 align-middle display-faded">{{
                      $t("closedSource")
                    }}</span>
                  </template>
                </span>
                <span class="ltr:mr-4 rtl:ml-4">
                  <template v-if="app.free">
                    <i
                      class="ri-creative-commons-nc-fill text-2xl align-middle text-josa-teal"
                    ></i
                    ><span class="text-sm mx-1 align-middle">{{
                      $t("free")
                    }}</span>
                  ...
                  <template v-else>
                    <i
                      class="ri-creative-commons-nc-fill text-2xl align-middle display-faded"
                    ></i
                    ><span class="text-sm mx-1 align-middle display-faded">{{
                      $t("commercial")
                    }}</span>
                ...
                <span
                  class="ltr:mr-4 rtl:ml-4 flex flex-nowrap items-center"
                  v-if="app.endorsed"
                >
                  <img
                    src="/images/logo/josa-icon-teal.svg"
                    class="josa-icon inline"
                  /><span class="text-sm mx-1 align-middle">{{
                    $t("endorsed")
                  }}</span>
                </span>
          ...
          <div class="flex flex-row flex-wrap md:flex-nowrap my-12">
            <p
              class="display-excerpt mb-8 md:ltr:mr-8 md:rtl:ml-8 flex-grow"
              v-if="app['description_' + $i18n.locale]"
            >
              {{ app["description_" + $i18n.locale] }}
            </p>
            <div class="flex flex-col justify-end min-w-max max-w-min">
              <a
                v-if="app.app_publisher.website_url"
                :href="app.app_publisher.website_url"
                target="_blank"
